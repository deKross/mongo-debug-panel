...
class Bunch(defaultdict):
    def __init__(self, default_factory=None, **kwargs):
        if default_factory is None:
            default_factory = Bunch
        super(Bunch, self).__init__(default_factory, **kwargs)
...
total = Bunch()
total.queries = Bunch(time=0, count=0, getmore_time=0, getmore_count=0)
total.collections = Bunch(lambda: Bunch(time=0, count=0))
...
def _update(collection_self, spec, document, upsert=False,
           manipulate=False, safe=False, multi=False, **kwargs):
    start_time = time.time()
...
def _cursor_refresh(cursor_self):
    ...
    if is_getmore:
        ...
        total.queries.getmore_count += 1
        total.queries.getmore_time += query_data['time']
        return result
    ...
    total.queries.count += 1
    total.queries.time += query_data['time']
    total.collections[query_data['collection']].count += 1
    total.collections[query_data['collection']].time += query_data['time']
    return result
...
def mofilter(argument, kind='collection', inverted=False, source=queries, only=None):
    import operator
    if kind == 'time':
        _op = operator.ge
    else:
        _op = operator.eq
    if inverted:
        op = lambda a, b: not _op(a, b)
    else:
        op = _op
    return [(entry.get(only) if (only is not None and only in entry) else entry)
             for entry in source
              if (kind in entry and op(argument, entry[kind]))]
